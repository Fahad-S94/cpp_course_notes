...
int main(){
    ...
    //* A Pointer is a special kind of variable that is going to stores the addresses to other variables, and if your pointer is a pointer to int, the variables whose addresses you can store can only be int. 

    /*
    //* Declare and initialize pointer
    int* p_number {}; //* Will initialize with nullptr(nullpointer)
    ...
    //* It doesn't matter what type your point to, the pointer to it(type) will always be 8bytes of memory
    ...
    int* p_number6{}, other_int_var6{};	//* Confusing as you wonder if other_int_var6 is also a pointer to int. They are not. They are stored as regular int variables.
    ...
    */
    ...
    // So, int *p_int{&int_var} stores the address of int_var variable to pointer p_int
    ...
    //! Pointer To Char
    // Pointer to char divides the char into an array of individual characters and stores the address of the first value.
    ...
    //! Program Memory Map
    //A typical memory representation of a C program consists of the following 5 sections.
    ...
    */
    ...
    // This memory belongs to our program from now on. The system can't use it for anything else, until we return it. After this line executes, we will have a valid memory location allocated. The size of the allocated memory will be such that it can store the type pointed to by the pointer
    ...
    delete p_number4;
    p_number4 = nullptr; //good practice to reset memory to nullptr
    */
    ...
    std::cout << "p_number6 : " << p_number6 << std::endl; //points to some address
    std::cout << "*p_number6 : " << *p_number6 << std::endl; //valid value, int at initialization
    ...
    delete p_number5;
    delete p_number5; //compiles but the program will crash
    
    //! Dangling Pointers
    //A dangling pointer is a variable in a programming language that points to a memory location that is no longer valid or has been deallocated. This can happen when a programmer creates and uses an object, then frees it without changing the pointer value. Three types(cases)
    ...
    */
    ...
    std::cout << "*p_number1(after delete) : " << *p_number1 << std::endl; //* can't use deleted pointed. CRASH
    */
    ...
    //* Deleting p_number3 (keeping *p_number4)
    delete p_number3;
    ...
    */
    ...
    delete p_number7;
    p_number7 = nullptr; //* Reset the pointer after deletion
    ...
    */
    ...
    //* Solution 3 //ideal for dynamic memory allocation
    ...
    //! When New Fails
    //* will compile but the value/array is too large for the program to run causing the heap to run out
    /*
    ...
    */
    ...
    //* exception mechanism
    //* try and catch using "std::exception& ex"
    /*
    ...
    */
    ...
    //* std::nothrow // ideal for when you don't want exceptions thrown when new fails
    for(size_t i{0} ; i < 100 ; ++i){
    ...
    */
    ...
    delete p_number;
    nullptr; //* always do this for any null pointers
    ...
    /*
    int *p_number76 {new int{67}}; // Points to some address, let's call that address1
    ...
    */
    ...
    /*
    for( auto s : p_scores1){ //won't work because p_scores1 is just some location on the memory(pointer)
        std::cout << "value : " << s << std::endl;
    }
    */
